...
VERSION :: "1.92.8"
VERSION_NUM :: 19280
// TextureID_Invalid :: (ImTextureID)0 //[??]
...
// ARRAYSIZE :: IM_COUNTOF // RENAMED IN 1.92.6: IM_ARRAYSIZE -> IM_COUNTOF //[??]
...
Tab_Bar_Flag :: enum i32 {
    ...
    Fitting_Policy_Mixed              = 7, // Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then enable scrolling. Setting TabMinWidthShrink to FLT_MAX makes this behave like ImGuiTabBarFlags_FittingPolicyScroll.
...
Backend_Flag :: enum i32 {
    ...
    Has_Parent_Viewport        = 13, // Backend Platform supports honoring viewport->ParentViewport/ParentViewportId value, by applying the corresponding parent/child relationship at the Platform level. Child windows always appear in front of their parent window.
}
...
Col :: enum i32 {
    ...
    Checkbox_Selected_Bg         = 19, // Checkbox background when Selected, otherwise use FrameBg
    Slider_Grab                  = 20,
    ...
    Separator_Active             = 30,
    Resize_Grip                  = 31, // Resize grip in lower-right and lower-left corners of windows.
    Resize_Grip_Hovered          = 32,
    Resize_Grip_Active           = 33,
    Input_Text_Cursor            = 34, // InputText cursor/caret
    Tab_Hovered                  = 35, // Tab background, when hovered
    Tab                          = 36, // Tab background, when tab-bar is focused & tab is unselected
    Tab_Selected                 = 37, // Tab background, when tab-bar is focused & tab is selected
    Tab_Selected_Overline        = 38, // Tab horizontal overline, when tab-bar is focused & tab is selected
    Tab_Dimmed                   = 39, // Tab background, when tab-bar is unfocused & tab is unselected
    Tab_Dimmed_Selected          = 40, // Tab background, when tab-bar is unfocused & tab is selected
    Tab_Dimmed_Selected_Overline = 41, //..horizontal overline, when tab-bar is unfocused & tab is selected
    Docking_Preview              = 42, // Preview overlay color when about to docking something
    Docking_Empty_Bg             = 43, // Background color for empty node (e.g. CentralNode with no window docked into it)
    Plot_Lines                   = 44,
    Plot_Lines_Hovered           = 45,
    Plot_Histogram               = 46,
    Plot_Histogram_Hovered       = 47,
    Table_Header_Bg              = 48, // Table header background
    Table_Border_Strong          = 49, // Table outer and header borders (prefer using Alpha=1.0 here)
    Table_Border_Light           = 50, // Table inner borders (prefer using Alpha=1.0 here)
    Table_Row_Bg                 = 51, // Table row background (even rows)
    Table_Row_Bg_Alt             = 52, // Table row background (odd rows)
    Text_Link                    = 53, // Hyperlink color
    Text_Selected_Bg             = 54, // Selected text inside an InputText
    Tree_Lines                   = 55, // Tree node hierarchy outlines when using ImGuiTreeNodeFlags_DrawLines
    Drag_Drop_Target             = 56, // Rectangle border highlighting a drop target
    Drag_Drop_Target_Bg          = 57, // Rectangle background highlighting a drop target
    Unsaved_Marker               = 58, // Unsaved Document marker (in window title and tabs)
    Nav_Cursor                   = 59, // Color of keyboard/gamepad navigation cursor/rectangle, when visible
    Nav_Windowing_Highlight      = 60, // Highlight window when using Ctrl+Tab
    Nav_Windowing_Dim_Bg         = 61, // Darken/colorize entire screen behind the Ctrl+Tab window list, when active
    Modal_Window_Dim_Bg          = 62, // Darken/colorize entire screen behind a modal window, when one is active
}
...
Style_Var :: enum i32 {
    ...
    Drag_Drop_Target_Rounding       = 35, // float     DragDropTargetRounding
    Button_Text_Align               = 36, // ImVec2    ButtonTextAlign
    Selectable_Text_Align           = 37, // ImVec2    SelectableTextAlign
    Separator_Size                  = 38, // float     SeparatorSize
    Separator_Text_Border_Size      = 39, // float     SeparatorTextBorderSize
    Separator_Text_Align            = 40, // ImVec2    SeparatorTextAlign
    Separator_Text_Padding          = 41, // ImVec2    SeparatorTextPadding
    Docking_Separator_Size          = 42, // float     DockingSeparatorSize
}
...
Font_Flag :: enum i32 {
    ...
    Implicit_Ref_Size = 4, // [Internal] Reference size was not set explicitly.
}
...
// Font_Atlas_Custom_Rect :: ImFontAtlasRect //[??]
...
Style :: struct {
    ...
    tab_close_button_min_width_selected:   f32,             // -1: always visible. 0.0f: visible when hovered. >0.0f: visible when hovered if minimum width. FLT_MAX: never shrink, will behave like ImGuiTabBarFlags_FittingPolicyScroll.
    ...
    drag_drop_target_rounding:             f32,             // Radius of the drag and drop target frame. When <0.0f: use FrameRounding.
    ...
    separator_size:                        f32,             // Thickness of border in Separator(). Must be >= 1.0f.
...
Input_Text_Callback_Data :: struct {
    ...
    cursor_pos:      i32,              //                                      // Read-write   // [Completion,History,Always,CharFilter]
    selection_start: i32,              //                                      // Read-write   // [Completion,History,Always,CharFilter] == to SelectionEnd when no selection
    selection_end:   i32,              //                                      // Read-write   // [Completion,History,Always,CharFilter]
}
...
Window_Class :: struct {
    ...
    platform_icon_data:            rawptr,          // [EXPERIMENTAL] Pass opaque data for Platform backend to handle.
}
...
Viewport :: struct {
    ...
    platform_icon_data:      rawptr,         // void* to hold custom data structure for the OS / platform to specify an icon. Currently unused for exposed to allow experiments.
...
Platform_IO :: struct {
    ...
    draw_callback_reset_render_state:      Draw_Callback,                                                                              // Request to reset the graphics/render state.
    draw_callback_set_sampler_linear:      Draw_Callback,                                                                              // Request backend to set texture sampling to Linear.
    draw_callback_set_sampler_nearest:     Draw_Callback,                                                                              // Request backend to set texture sampling to Nearest/Point.
...
foreign lib {
    ...
    image :: proc(tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}) ---
    @(link_name = "ImGui_ImageWithBg")
    image_with_bg :: proc(tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) ---
    @(link_name = "ImGui_ImageButton")
    image_button :: proc(str_id: cstring, tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) -> bool ---
    ...
    begin_popup_context_item :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    // open+begin popup when clicked on current window.
    @(link_name = "ImGui_BeginPopupContextWindow")
    begin_popup_context_window :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    // open+begin popup when clicked in void (where there are no windows).
    @(link_name = "ImGui_BeginPopupContextVoid")
    begin_popup_context_void :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    ...
    set_item_key_owner :: proc(key: Key) -> bool ---
    ...
    draw_list_add_line_h :: proc(self: ^Draw_List, min_x: f32, max_x: f32, y: f32, col: u32, thickness: f32 = 1.0) ---
    @(link_name = "ImDrawList_AddLineV")
    draw_list_add_line_v :: proc(self: ^Draw_List, x: f32, min_y: f32, max_y: f32, col: u32, thickness: f32 = 1.0) ---
    // a: upper-left, b: lower-right (== upper-left + size)
    @(link_name = "ImDrawList_AddRect")
    draw_list_add_rect :: proc(self: ^Draw_List, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, thickness: f32 = 1.0, flags: Draw_Flags = {}) ---
    // a: upper-left, b: lower-right (== upper-left + size)
    ...
    draw_list_add_polyline :: proc(self: ^Draw_List, points: ^Vec2, num_points: i32, col: u32, thickness: f32, flags: Draw_Flags = {}) ---
    ...
    im_draw_list_add_image :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p_min: Vec2, p_max: Vec2, uv_min: Vec2 = Vec2{0, 0}, uv_max: Vec2 = Vec2{1, 1}, col: u32 = 0xff_ff_ff_ff) ---
    @(link_name = "ImDrawList_AddImageQuad")
    im_draw_list_add_image_quad :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1: Vec2 = Vec2{0, 0}, uv2: Vec2 = Vec2{1, 0}, uv3: Vec2 = Vec2{1, 1}, uv4: Vec2 = Vec2{0, 1}, col: u32 = 0xff_ff_ff_ff) ---
    ...
    draw_list_path_stroke :: proc(self: ^Draw_List, col: u32, thickness: f32 = 1.0, flags: Draw_Flags = {}) ---
    ...
    draw_list_add_callback :: proc(self: ^Draw_List, callback: Draw_Callback, userdata: rawptr = nil, userdata_size: uint = {}) ---
    ...
    font_get_font_baked :: proc(self: ^Font, font_size: f32, density: f32 = -1.0) -> ^Font_Baked ---
    @(link_name = "ImFont_CalcTextSizeA")
    font_calc_text_size_a :: proc(self: ^Font, size: f32, max_width: f32, wrap_width: f32, text_begin: cstring, text_end: cstring = nil, out_remaining: ^cstring = nil) -> Vec2 ---
    ...
    font_render_char :: proc(self: ^Font, draw_list: ^Draw_List, size: f32, pos: Vec2, col: u32, c: Wchar, cpu_fine_clip: ^Vec4 = nil) ---
